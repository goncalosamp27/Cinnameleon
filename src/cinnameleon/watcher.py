...
class WallpaperWatcher:
    ...
    def apply_profile(
        ...
        """Manually apply a configured profile."""
    ...
    def apply_effective_profile(
        self,
        profile: Profile,
        effective_profile: EffectiveProfile,
    ) -> bool:
        """
        Apply a temporary already-resolved profile.

        Used by the graphical editor before values
        are saved to YAML.
        """

        if profile.id != effective_profile.id:
            self._emit(
                "Temporary profile ID does not "
                "match the configured profile."
            )
            return False

        self._mode = effective_profile.mode

        # Prevent the wallpaper change generated by
        # SettingsBackend from immediately reloading the
        # YAML version of the same profile.
        self._last_wallpaper = (
            profile.wallpaper.resolve()
        )
    ...
    def synchronize_current_wallpaper(
        self,
    ) -> bool:
        ...
        return self._synchronize_wallpaper(
            wallpaper
        )
    ...
    def _synchronize_wallpaper(
        ...
        if self._processing:
            self._emit(
                "Wallpaper event ignored while "
                "another profile is being applied."
            )
            return False
    ...
    def _on_picture_uri_changed(
        ...
        if wallpaper is None:
            self._emit(
                "Wallpaper changed, but its URI "
                "is not a supported local file."
            )
        ...
        self._emit(
            f"Wallpaper changed: {wallpaper}"
        )

        self._synchronize_wallpaper(
            wallpaper
        )
    ...
    def start_listening(
        ...
        *,
        ...
        self._signal_id = (
            self._background_settings.connect(
    ...
    def _apply_effective(
        ...
        if self._processing:
            self._emit(
                "Profile selection ignored while "
                "another profile is being applied."
            )
